...
```{r}
temp_rho <- temp_rho %>% mutate(prop_ce = school /(school + 1) * 100, 
                    prop_fora = 100 - prop_ce)
fig <- plot_ly(temp_rho, x = ~ date)
...
temp <- rho_average %>%
  mutate(
    prop_esc = Escola / (Escola + 1) * 100,
    prop_fora_esc = 100 - prop_esc,
    prop_inst = Institut / (Institut + 1) * 100,
    prop_fora_inst = 100 - prop_inst,
    name = ymd(name)
  )
```
