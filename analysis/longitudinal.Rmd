...
# fig <- fig %>% add_trace(y = ~ Guarderia, name = "Guarderia", type = "scatter", mode = "lines")
...
Abans, però, notem que el valor màxim d'aquesta corba projectada és `r round(max(traj$I))`, que serien el nombre d'infants i adolescents que es contagiarien segons aquest model. Segons aquest model, el màxim seria el dia `start_schools + which.max(traj$I)`. Hem de tenir en compte que aquí es nota força que les dades són de 12 dies enrere, i per tant aquest model segurament evolucioni amb el temps. 
...
\[ \rho_T = \frac{\rho_E r + \rho_P}{1 + r} \], o, aïllant $\rho_P$, que és la que ens interessa ara:
